...
import { saveNotifications, notificationExists, getNotification } from './service/firestoreService.js';
  ...
  try {
    ...
    for (const doc of results) {
      try {
        ...
        if (exists) {
          console.log(`Benachrichtigung ${bgblNummer} existiert bereits in Firestore, lade aus Datenbank.`);
          
          // Lade die bestehende Benachrichtigung aus Firestore
          const existingNotification = await getNotification(bgblNummer);
          
          if (existingNotification) {
            console.log(`Benachrichtigung ${bgblNummer} erfolgreich aus Firestore geladen.`);
            notifications.push({
            ...
            console.log(`Konnte Benachrichtigung ${bgblNummer} nicht aus Firestore laden, nutze Basisdaten.`);
            notifications.push({
              id: bgblNummer,
        ...
        console.log(`Neue Benachrichtigung ${bgblNummer} gefunden, verarbeite XML...`);
    ...
    if (newNotifications.length > 0) {
      try {
        ...
        console.log(`${newNotifications.length} neue Benachrichtigungen erfolgreich in Firestore gespeichert`);
      } catch (firestoreErr) {
        console.error('Fehler beim Speichern in Firestore:', firestoreErr.message);
      }
    } else {
...
app.get('/api/stored-notifications', async (req, res) => {
  try {
    const { getAllNotifications } = await import('./service/firestoreService.js');
    ...
    console.log(`${notifications.length} Benachrichtigungen aus Firestore geladen.`);
    ...
    console.error("Fehler beim Abrufen der gespeicherten Benachrichtigungen:", err.message);
    res.status(500).json({
      success: false,
      error: err.message
    });
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log(`- GET /api/stored-notifications - Gespeicherte Benachrichtigungen aus Firestore abrufen`);
